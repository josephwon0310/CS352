...
class SemanticAnalyzer(parser: Parser) extends Reporter {
  ...
  def analyze(exp: Exp)(env: VarEnv): Unit = exp match {
    ...
    case Prim(op, lop, rop) =>
      ...
      analyze(rop)(env)
      analyze(lop)(env) // TODO
    case Let(x, a, b) =>
      // TODO: check variable reuse
      if (env.isDefined(x))
        warn("Already defined variable", b.pos)
      analyze(a)(env) 
    ...
    case Cond(op, l, r) =>
      if (!isBOperator(op))
        error("undefined binary operator", exp.pos) // TODO
      analyze(l)(env)
      analyze(r)(env)
