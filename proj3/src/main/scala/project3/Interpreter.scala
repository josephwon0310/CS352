...
class StackInterpreter extends Interpreter with BugReporter {
  ...
  def eval(exp: Exp, sp: Loc)(env: LocationEnv): Unit = exp match {
    ...
    case LetRec(funs, body) => 
      ...
      var funcs = List[(String, Int)]()

      (funs zip idxs) foreach {
        case (fun, idx) => 
          // case fun2@FunDef(name, _,_,_) =>
            eval(fun, idx)(env)
            funcs = funcs :+ (fun.asInstanceOf[FunDef].name, idx)
      }

      funcs foreach { case (_, idx) => memory(idx).asInstanceOf[Func].withVals(funcs) }
      
      eval(body, sp + idxs.size + 1)(env.withVals(funcs))
      memory(sp) = memory(sp + idxs.size + 1)
      
    case App(fun, args) => 
      val idxs = List.tabulate(args.length)(i => sp+i)
      var nargs = List[(String, Int)]()
      ...
      var new_sp = sp+idxs.size+1

      eval(fun, new_sp)(env)

      memory(new_sp) match {
        case Func(fargs, fbody, fenv) =>
          eval(fbody, new_sp)(fenv.withVals(fargs zip idxs))
          memory(sp) = memory(new_sp)
        case Cst(_) =>
          ???
      }
