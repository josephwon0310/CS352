...
class BaseParser(in: Scanner) extends Parser(in) {
  ...
  def parseCode = {
    ...
    if (in.hasNext)
      expected(s"EOF")
    LetRec(Nil, res).withPos(in.peek.pos).asInstanceOf[LetRec]
  }
  ...
  def parseSimpleExpression: Exp = (in.peek, in.peek1) match {
    ...
    case (Keyword("if"), _) =>
      ...
      val elpos = accept("else").pos
      val eBranch = parseSimpleExpression.withPos(elpos)
...
class SyntacticSugarParser(in: Scanner) extends BaseParser(in) {
  ...
  override def parseSimpleExpression = in.peek match {
    case Keyword("if") =>
        ...
        if (in.peek == Keyword("else")) {
            val elsepos = accept("else").pos
            val eBranch = parseSimpleExpression
            If(cond, tBranch, eBranch).withPos(elsepos)
        } else {
            val eBranch = Lit(())
            If(cond, tBranch, eBranch).withPos(pos)
        }
  ...
  override def parseExpression = {
    ...
    if (isNewLine(in.peek)) {
        val pos = accept(';').pos
        val dum = parseExpression
        Let(freshName("tmp"), parseOptionalType, res, dum).withPos(pos)
    } else {
      res
    }
  }
...
class FunctionParser(in: Scanner) extends SyntacticSugarParser(in) {
  ...
  def parseList[T](parseElem: => T, sep: Char, cond: Token => Boolean, first: Boolean = true): List[T] = {
    if (first && cond(in.peek) || (!first && in.peek == Delim(sep) && cond(in.peek1))) {
      if (!first) {
        //println("should be accepting here")
        accept(sep)
      }
  ...
  override def parseType = in.peek match {
    case Ident(x) =>
      val itp = super.parseType
      if (in.peek == Keyword("=>")) {
        var tparg = List[(String, Type)]()
        tparg = ("", itp) :: tparg
        accept("=>")
        val rtp = parseType
        FunType(tparg, rtp)
    ...
    case Delim('(') =>
      accept('(')
      var tparg = List[(String, Type)]()
      val tps = parseList[Type](parseType, ',', tok => tok match {
        case Ident(tok) => true;
        case _ => false
      })
      for (x <- tps) {
  ...
  def parseFunction: Exp = {
    val pos1 = accept("def").pos
    val (name, pos2) = getName
    accept('(')
    var args = parseList[Arg](parseArg, ',' , tok => tok match {
      case Ident(tok) => 
        //println(tok)
        true;
      case _ => 
        // print("Ident(tok) expected but given: ")
        // println(tok)
        false
    })
    accept(')')
    ...
    FunDef(name, args, rtp, fbody).withPos(pos2)
  }
  ...
  def parseProgram = in.peek match {
    case Keyword("def") =>
      var funcs = parseList[Exp](parseFunction, ';', tok => tok match {
        case Keyword("def") => true;
        case _ => false
      })
      val pos = accept(';').pos
      LetRec(funcs, parseExpression).withPos(pos).asInstanceOf[LetRec]
    case _ => LetRec(Nil, parseExpression).withPos(in.peek.pos).asInstanceOf[LetRec]
  }
  ...
  def parseTight = in.peek match {
    ...
    case  _ =>
      ...
      if (in.peek == Delim('(')) {
        ...
        while (in.peek == Delim('(')) {
          val pos = accept('(').pos
          val simps = parseList[Exp](parseSimpleExpression, ',', tok => tok match {
            case Delim(')') => false;
            case _ =>
              true
          })
          accept(')')
          res = App(res, simps).withPos(pos)
        }
        res.withPos(in.peek.pos)
      } else {
        res.withPos(in.peek.pos)
      }
      // TODO: complete
...
class ArrayParser(in: Scanner) extends FunctionParser(in) {
  ...
  override def parseType = in.peek match {
    case Keyword("Array") => //TODO: THIS MIGHT NEED TO CHANGE
      val pos = accept("Array").pos
      accept('[')
      val tp = parseType
      accept(']')
      ArrayType(tp)
    ...
    case _ =>
      ...
      if (in.peek == Delim('(')) {
        ...
        if (in.peek == Delim('=')) {
          ...
          val pos = accept('=').pos
          val simp2 = parseSimpleExpression
          //var wrap = List[Exp]()
          wrap = wrap :+ simp2
          //App(res, wrap)
          Prim("block-set", wrap).withPos(pos)
           
        } else { //basically super.parseTight, just return what we have
          res.withPos(in.peek.pos)
        }
      } else {
        res.withPos(in.peek.pos)
  ...
  override def parseSimpleExpression = in.peek match {
    case Keyword("new") =>
      ...
      accept('(')
      //println(in.peek)
      val simp1 = parseSimpleExpression
      //println(simp1)
      accept(')')
      ArrayDec(simp1, arrtype).withPos(newpos)
    case _ =>
